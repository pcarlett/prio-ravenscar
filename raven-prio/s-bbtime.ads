...
package System.BB.Time is
   ...
   --  The Time package uses these facilities to keep a 64-bit clock that
   --  will allow to keep track of up to 50 years in the future without
   --  having the most significant bit set. This means it is always safe
   --  to subtract two Clock readings to determine a Time_Span without
   --  overflow.
   ...
   --  seconds. As comparison, a LEON2 at 80 MHz with 24-bit clock and
   --  the minimum prescale factor of 4, has a period of 2**24 / (80E6 /
   --  4) = 0.839 seconds, while a 200 MHz LEON3 has a period of 2**32 /
   --  (200E6 / 5) = 107 seconds.  For faster clocks or smaller clock width,
   --  higher prescaler values may be needed to achieve 50 year run time.
   --  The prescale factor should be chosen such that the period between
   --  clock ticks is an integral number of nanoseconds between 1 and 20_000.
   ...
   --  requirement is that an alarm always can be handled within 1/8th of
   --  the time it takes the hardware clock to wrap around. This gives an
   --  upper bound to how early we have to set the alarm to ensure timely
   --  clock updates. This will result in an interrupt rate 14% higher than
